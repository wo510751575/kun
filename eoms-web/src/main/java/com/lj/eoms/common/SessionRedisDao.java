...
public class SessionRedisDao extends AbstractSessionDAO implements SessionDAO {

	@Autowired
	private com.lj.distributecache.RedisCache redisCache; // 记录并区分添加方式
	...
	public void update(Session session) throws UnknownSessionException {
		...
		try {
			...
			jedis.set(RedisCache.getBytesKey(sessionKeyPrefix + session.getId()), RedisCache.toBytes(session));
	...
	public void delete(Session session) {
		...
		try {
			...
			jedis.hdel(RedisCache.getBytesKey(sessionKeyPrefix), RedisCache.getBytesKey(session.getId().toString()));
			jedis.del(RedisCache.getBytesKey(sessionKeyPrefix + session.getId()));
			...
			for (Map.Entry<String, String> e : map.entrySet()) {
				if (StringUtils.isNotBlank(e.getKey()) && StringUtils.isNotBlank(e.getValue())) {
					...
					if (ss != null && ss.length == 3) {// jedis.exists(sessionKeyPrefix + e.getKey())){
						// Session session =
						// (Session)redisCache.toObject(jedis.get(redisCache.getBytesKey(sessionKeyPrefix
						// + e.getKey())));
...
//				if (jedis.exists(sessionKeyPrefix + sessionId)){
			session = (Session) RedisCache.toObject(jedis.get(RedisCache.getBytesKey(sessionKeyPrefix + sessionId)));
//				}
