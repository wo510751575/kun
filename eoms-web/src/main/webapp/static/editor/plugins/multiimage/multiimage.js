	...
	init : function(options) {
		...
		var settings = {
			...
			upload_success_handler : function(file, serverData) {
				...
				if (data.url == null) {
					showError(itemDiv, K.DEBUG ? data.message.content : self.options.errorMessage);
					return;
				}
				data.url = uploadUrl+'/eoms'+data.url;
				file.url = data.url;
				data.error=0;
		...
		for (var name in postParams) {
			if (postParams.hasOwnProperty(name)) {
				if(postParams[name]==null){
					paramStringPairs.push(encodeURIComponent(name.toString()) + "=" + encodeURIComponent(""));
				}else{
